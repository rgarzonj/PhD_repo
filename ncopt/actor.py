...
#            delta_y2 = tf.transpose(tf.square(ordered_y_[1:]-ordered_y_[:-1]),[1,0]) # [batch_size, seq length]        delta_y**2
        ...
        with tf.name_scope('environment'):
            ...
            unst = tf.unstack(tf.unstack(self.input_,axis=0),axis=0)
            #print (len(unst))
            #print ('*****')
            #print (unst)
            #print (unst[0])
            numBlocks = int(unst[0].shape[0]//2)
            #print (numBlocks)
            #print (type(numBlocks))

            rewards = []
            for oneSequence in range(self.batch_size):
                reward = 0
                #print (unst[oneSequence].shape)
                #oneSequence has shape (8,1)
                current = tf.slice(unst[oneSequence],[0,0],[numBlocks,1])
                #print (current.shape)
                goal = tf.slice(unst[oneSequence],[numBlocks,0],[numBlocks,1])
                #print (goal.shape)
                #Now we have current state and goal
                #Compute how many items are different 
                reward = tf.reduce_sum(tf.cast(tf.equal(current,goal), tf.float32))-numBlocks
                rewards.append(reward)
            self.reward = tf.stack(rewards)
