...
def make_input_fn(
    ...
    def input_fn():
        ...
        outp = tf.placeholder(tf.int64, shape=[None, None], name='output')
        tf.identity(inp[0], 'input_0')
        tf.identity(outp[0], 'output_0')
        return {
            'input': inp,
            'output': outp,
        }, None
...
def main():
    ...
    ret = predict_seq2seq('input_eval', 'output_eval', 'vocab', 'model/seq2seq')
    print (ret)
